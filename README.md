...
Emulator의 WHPX/gfxstream 엔진과 최소 AVD를 사용합니다.

> 이 프로젝트는 비공식 커뮤니티 프로젝트입니다. Pokémon, Pokémon Champions,
> Google, Android 또는 각 권리자와 제휴·후원·승인 관계가 없습니다.
...
- Android 홈과 오른쪽 Emulator 툴바를 숨기고 이동 가능한 1280×720 게임 창과 오디오만 표시합니다.
- scrcpy 영상 인코딩 없이 Emulator의 host GPU 표면을 직접 표시하며 30Hz, 5 vCPU, 4GB RAM으로 실행합니다.
- 게임 실행 전과 게임 창이 열린 뒤 세 차례 Play 스토어의 예약 다운로드 작업을 취소합니다.
- 게임에 불필요한 음성 검색·TTS·Android System Intelligence의 백그라운드 모델 다운로드를 비활성화합니다.
...
Android Studio, JDK, Android SDK/Emulator와 Google Play 시스템 이미지를
...
온라인 세션 만료와 복원 오류를 피하기 위해 Emulator 상태 스냅샷을 저장하거나
불러오지 않고 매 실행마다 콜드부팅합니다.
...
```powershell
Get-FileHash .\andchamps-launcher-v0.1.2-win-x64.zip -Algorithm SHA256
...
.\scripts\Verify-Release.ps1 `
  -ArtifactPath .\andchamps-launcher-v0.1.2-win-x64.zip `
...
- `artifacts\release\andchamps-launcher-v0.1.2-win-x64.zip`
...
이 저장소의 자체 소스는 [MIT License](LICENSE)로 배포합니다. .NET,
